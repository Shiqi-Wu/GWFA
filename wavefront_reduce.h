...
void AddWavefront(Wavefront& w, WaveStruct& s, bool M_in[m + 1][n + 1])	//add the wavefront Index w to s
{
	...
	if (!M_in[w.h][w.u])
	{
		...
		w.next = q;
		s.next = &w;
		s.max_h = s.max_h > w.h ? s.max_h : w.h;
...
void WFReduction(Graph& q, int tran_string[], penalty p, WaveStruct M[], WaveStruct I[], Wavefront D[], int gap, int s)
{
	if (M[s].max_h - gap > 0)
	{
		int lower = M[s].max_h - gap;
		Wavefront* p = M[s].next;
		while (p)
		{
			if (p->h < lower)
			{
...
void WFGraphAlign(Graph& q, int tran_string[], penalty p, int gap)
{
	...
	{
		M[k].score = k; M[k].next = NULL; M[k].max_h = 0;
		I[k].score = k; I[k].next = NULL; I[k].max_h = 0;
		D[k].score = k; D[k].next = NULL; D[k].max_h = 0;
	}
	...
	*/
	...
	while (1)
	{
		...
		WFReduction(q, tran_string, p, M, I, D, gap, s);
		w = new Wavefront;
		w->h = m;
		w->u = n;
		w->next = NULL;
